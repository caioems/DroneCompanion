...
class HealthTests:
    ...
    def vibe_test(self):
        ...
        clip_columns = ["Clip0", "Clip1", "Clip2"]
        available_columns = [
            col for col in clip_columns if col in self._vibe_df.columns
        ]

        clips = [self._vibe_df[col].iloc[-1] for col in available_columns]
    ...
    def vcc_test(self):
        ...
        if self.vcc_mean < vcc_fail:
            self.vcc_status = "FAIL"
            self.vcc_feedback = f"Internal board voltage is too low! ({self.vcc_mean}v, ±{self.vcc_std}v)"
