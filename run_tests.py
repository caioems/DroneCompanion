...
if __name__ == "__main__":
    ...
    for i in tqdm(ppl._log_list):
      ppl.run(i)
    # with concurrent.futures.ThreadPoolExecutor(max_workers=int(len(ppl._log_list))) as executor:
    #    tasks = list(tqdm(executor.map(ppl.run, ppl._log_list), total=len(ppl._log_list)))
    #    print(tasks)
